package pet.com.br.pet.utils;

/**
 * Created by iaco_ on 27/08/2016.
 */
public class ChatUtils {

    public static final String DATA_URL = "http://thebossgamestudio.xyz/pet/getChat.php?page=";
    public static final String DATA_NEW_URL = "http://thebossgamestudio.xyz/pet/getNewChat.php?page=";
    public static final String TAG_CODIGO = "CODIGO";
    public static final String TAG_USERCHAT = "USUARIOCLIENTE";
    public static final String TAG_DESCRICAO = "DESCRICAO";




}
